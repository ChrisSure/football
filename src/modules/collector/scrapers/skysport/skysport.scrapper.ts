...
export class SkySportScraper implements Scraper {
  ...
  private extractLinks($: CheerioAPI): string[] {
    ...
    $('[data-testid="sitewide-tiles-headline-link"]').each((_index, element) => {
      const href = $(element).attr('href');
      if (href && href.includes('/football/news/') && !href.includes('push-notifications-faq')) {
        links.push(href);
      }
    });
  ...
  private async parseArticle(link: string, source: Source): Promise<Article | null> {
    ...
    const title = page('.sdc-site-component-header--h1 span').html() ?? '';
    const image = page('.sdc-article-image__item').attr('src') ?? '';
