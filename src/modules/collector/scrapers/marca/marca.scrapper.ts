...
export class MarcaScraper implements Scraper {
  ...
  private extractLinks($: CheerioAPI): string[] {
    ...
    $('.ue-c-cover-content__link').each((_index, element) => {
  ...
  private async parseArticle(link: string, source: Source): Promise<Article | null> {
    ...
    const title = page('.ue-c-article__headline').html() ?? '';
    const image = page('.ue-c-article__image').attr('src') ?? '';
